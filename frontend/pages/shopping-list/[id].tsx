...
const ShoppingListPage: React.FC = () => {
  ...
  useEffect(() => {
    const generateShoppingList = async () => {
      ...
      try {
        setLoading(true);
        const data = await apiService.generateShoppingList(id as string);
        
        // Add checked property to each item
        const listWithCheckedState = {
          ...data,
          items: data.items.map((item: ShoppingListItem) => ({
            ...item,
            checked: false
          }))
        };
        
        setShoppingList(listWithCheckedState);
      } catch (err) {
    ...
    return (
      <Container maxW="container.xl" centerContent py={10}>
        <VStack spacing={6}>
          ...
          <Text>Generating your shopping list...</Text>
        </VStack>
      </Container>
    );
  }
  ...
  if (error || !shoppingList) {
    return (
      <Container maxW="container.xl" centerContent py={10}>
        <VStack spacing={6}>
          <Heading color="red.500">Error Generating Shopping List</Heading>
          <Text>{error || 'Shopping list could not be generated'}</Text>
          <Button colorScheme="green" onClick={handleBackToMealPlan}>
            Back to Meal Plan
          </Button>
        </VStack>
      </Container>
